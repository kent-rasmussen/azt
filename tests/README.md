...
| `manual/webview_multiwindow/` | `python platform_probe.py` (also `two_windows.py`, `hide_show.py`) | Does this platform's webview support the windows A-Z+T needs — created-hidden then `show()`, hide/show round trips, `resize()`, and `destroy()` without taking the process with it? Drives each step from Python and draws a PASS/FAIL table **in the page**, so a machine that can't paste text still reports by photograph. Known: Linux/GTK fails created-hidden; Qt segfaults on destroy; **EdgeChromium (Windows) untested** — see `agenda/webview_when_to_finish.md`. |

...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:exsl="http://exslt.org/common">
   ...
   <xsl:variable name="chosenContentTypes" select="id(exsl:node-set($chosenContentControl)/@exclude)"/>
   ...
   <xsl:template name="IgnoreOrCopyLineInInterlinearText">
      ...
      <xsl:variable name="contentType" select="id(exsl:node-set($text)/@contentType)/@id"/>
      <xsl:variable name="inSet">
         <xsl:if test="$chosenContentTypes">
            <xsl:for-each select="$chosenContentTypes">
               <xsl:if test="@id = $contentType">x</xsl:if>
      ...
      <xsl:choose>
         ...
         <xsl:when test="string-length(exsl:node-set($text)/@linesToInclude) &gt; 0">
            <xsl:if test="contains(exsl:node-set($text)/@linesToInclude,concat(',',position(),','))">
               <xsl:copy>
                  <xsl:apply-templates select="@*"/>
                  <xsl:apply-templates/>
               </xsl:copy>

...
for /f "tokens=*" %%i in ('powershell -noprofile -command "[Environment]::GetFolderPath([Environment+SpecialFolder]::Desktop)"') DO (
SET desktop=%%i
)
...
(
echo @echo off
echo echo downloading A-Z+T
echo call :runme-2 ^> %~dpn0-2_log.txt 2^>^&1
echo exit /B
echo :runme-2
echo ECHO Cloning A-Z+T source to '%desktop%\azt'
echo echo going to run git clone %azt% ^"%desktop%\azt^"
echo git config --global --add safe.directory %azt%
echo git clone %azt% ^"%desktop%\azt^"
...
mklink "%desktop%\A-Z+T" "%desktop%\azt\main.py"
mklink "%desktop%\Transcriber" "%desktop%\azt\transcriber.py"
...
powershell.exe -noprofile -command "Expand-Archive %praatfilename% -Force -DestinationPath ""%ProgramFiles%"""

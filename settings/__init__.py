...
class Settings(SettingsUI):
    ...
    def polygraphLWCdefaults(self):
        ...
        if self.program.params.analang in lwcdefaults:
            _log.info(_("It looks like you're working on your LWC; using {lang} digraph defaults")
    ...
    def polygraphcheck(self):
        ...
        # _log.info("Language settings: {}".format(self.program.db.s))
        firstrun=False
        ...
        for lang in self.program.db.analangs:
            if lang not in self.program.db.s:
                _log.error(_("Language {lang} found without segment settings."
                            ).format(lang=lang))
                continue
            ...
            for sclass in [sc for sc in self.program.db.s[lang]
                ...
                for pg in self.program.db.s[lang][sclass]:
                    ...
                    if not firstrun and pg not in self.polygraphs[lang][pclass]:
                        _log.info(_("{polygraph} ({pclass}/{sclass}) has no Di/Trigraph setting; "
    ...
    def checkinterpretations(self):
        ...
        _log.log(2,"self.distinguish: {}".format(self.distinguish))
    def addtoprofilesbysense(self,sense,ps,profile):
        # _log.info("kwargs: {}".format(kwargs))
    ...
    def addtoformstosearch(self,sense,form,ps,oldform=None):
        ...
        # _log.info("Added {} id={}".format(form,sense.id))
        if oldform:
            try:
                self.formstosearch[ps][oldform].remove(sense)
                _log.info(_("Removed {form} ({val})").format(form=form,val=self.formstosearch[ps][form]))
            except ValueError:
                _log.error(_("Apparently {sense} isn't under {form}?").format(sense=sense, form=form))
            if not self.formstosearch[ps][oldform]:
                del self.formstosearch[ps][oldform] #don't leave form wo sense
                _log.info(_("Deleted key of empty list"))
    ...
    def getprofilesbyps(self,ps):
        ...
        # _log.info(_("Processing {ps} syllable profiles").format(ps=ps))
        senses=self.program.db.sensesbyps[ps]
        self.sextracted[ps]={} #start over, don't add to if there
        n=self._getprofiles(senses,ps)
        _log.info(_("Processed {n} forms to {ps} syllable profiles").format(n=n,ps=ps))
    ...
    def _getprofiles(self,senses,ps):
        ...
        for sense in senses:
            ...
            if n%100:
                ...
                _log.debug(_("{count}: {form} > {profile}").format(count=str(n)+'/'+str(todo),form=form,
                                            profile=profile))
            # if todo>750:
    ...
    def getprofiles(self):
        ...
        # _log.info("Interpretation: \n{}".format(
        #         '\n'.join([k+': '+self.interpret[k] for k in self.interpret])
        #         ))
        # _log.info("Distinguishing: \n{}".format(
        ...
        if hasattr(self,'adhocgroups'):
            for ps in self.adhocgroups:
                for a in self.adhocgroups[ps]:
                    _log.debug("Adding {} to {} ps-profile: {}".format(a,ps,
                    ...
                    # _log.debug("resulting profilesbysense: {}".format(
    ...
    def extractsegmentsfromform(self,form,ps):
        for s in set(self.profilelegit) & set(self.rxdict.rx):
            # _log.info('s: {}; rx: {}'.format(s, self.rxdict.rx[s]))
            # _log.info(f"srx output: {self.rxdict.rx[s][0].findall(form)}")
            for i in [j for j in self.rxdict.rx[s][0].findall(form) if j]:
                i=''.join(i).lower() #('o', 'e') > 'oe', no upper case
                # _log.info('found polygraph '{}''.format(i))
        ...
        for ps in self.sextracted: # was self.program.db.pss[self.analang]:
            ...
            for s in self.rxdict.rx:
                try:
                    ...
                    pass
                    # _log.info(_("{error} KeyError reading {ps}-{s} from sextracted"
    ...
    def slists(self):
        ...
        _log.info(_("Found db.analangs: {langs}").format(langs=self.program.db.analangs))
        _log.info(_("Found params analang: {lang}").format(lang=self.program.params.analang()))
        ...
        for lang in set(self.s)&set(self.program.db.s):
            ...
            for sclass in [x for x in self.program.db.s[lang]
                ...
                try:
                    ...
                    _log.debug(_("Polygraphs for {lang} in {sclass}: {pgs}").format(lang=lang,sclass=sclass,
                                                                    pgs=pgthere))
            ...
            _log.info(_("Segment lists for {lang} language: {segments}").format(lang=lang,
    ...
    def addtoCVrxs(self,s):
        ...
        if cvt in ['C', 'V'] and s not in self.s[analang][cvt]:
            ...
            # _log.info("Compiling rx list: {}".format(self.s[self.analang][cvt]))
            self.rxdict.makeglyphregex(cvt)
            # _log.info("Compiled rx list: {}".format(self.rx[cvt]))
    def compileCVrxforsclass(self,sclass):
        ...
        analang=self.program.db.analang
        # _log.info("compileCVrxforsclass RXs: {}".format(self.rx))
    def setupCVrxs(self):

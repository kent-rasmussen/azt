...
# coding=UTF-8
import langcodes
from data import ethnologue_language_relationships as iso
from data import ethnologue_macrolanguages_members
from data import mms_language_support
#Whisper support list comes from module

from utilities import rx,file,logsetup
import json,os
...
try:
    ...
    by_name={name:(langcodes.Language.get(code).to_alpha3(),code)
                                    for name,code in TO_LANGUAGE_CODE.items()}
    with open(file.localfile('whisper_codes_names.py'),'w') as f:
...
def whisper_languages(lang=None):
    ...
    """This comes from an installed module, so no reason to download it."""
    from transformers.models.whisper.tokenization_whisper import TO_LANGUAGE_CODE
    if lang:
...
class SisterLanguages(object):
    ...
    def load_mms_data(self):
        self.mms_data=mms_language_support.data
...
class Languages(dict):
    ...
    def load_json(self):
        try:
            with open(self.json_location, 'r', encoding="utf8") as f:
                data = json.load(f)

    ...
    def pssubclassvalue(self,subclass=None):
        try: # self.psnode would always evaluate False, as it has no child nodes
            ...
            if "object has no attribute 'ps'" in e.args[0]:
                log.info("no ps, so not setting pssubclass")
            elif "object has no attribute 'pssubclassnode'" in e.args[0]:
                self.pssubclassnode=ifone(self.sense.findall(
                                "trait[@name='{}-infl-class']".format(self.ps)
                                                            ))
                self.pssubclassvalue(subclass)
            elif "'NoneType' object has no attribute" in e.args[0]:
                if subclass: # Make one here
                    # <trait name='{ps}-infl-class' value='{pssubclass}'
                    self.pssubclassnode=lift.Node(
                                        self.sense,
                                        'trait',
                                        {'name':'{}-infl-class'.format(self.ps),
                                        'value':subclass})
                    log.info("created pssubclassnode senseid: {}"
                                "".format(self.senseid))
                    self.pssubclassvalue(subclass)
                    return
            else:
                log.info("pssubclassnode error! senseid: {}; {}"
                        "".format(self.senseid,e))
    def psvalue(self,ps=None):
        try: # self.psnode would always evaluate False, as it has no child nodes
            if ps:
                self.psnode.set('value',ps)
            self.ps=self.psnode.get('value')
        except AttributeError as e:
            if "object has no attribute 'psnode'" in e.args[0]:
                self.psnode=ifone(self.sense.findall('grammatical-info'))
                self.psvalue(ps)
            elif "'NoneType' object has no attribute" in e.args[0]:
                if ps: # Make one here
                    self.psnode=lift.Node(self.sense,
                                            'grammatical-info',
                                            {'value':ps})
                    log.info("created psnode senseid: {}".format(self.senseid))
                    self.psvalue(ps)
                else:
